...
Visible Procedure
debug1(string fmt, char c) {
	if (fmt && priority <= 1) {
		priority = 1;
		sprintf(msgbuffer, fmt, c);
	}
}

Visible Procedure
debug1i(string fmt, int i) {
	if (fmt && priority <= 1) {
		priority = 1;
		sprintf(msgbuffer, fmt, i);
	}
}

Visible Procedure
debug2(string fmt, int i, int j) {
	if (fmt && priority <= 1) {
		priority = 1;
		sprintf(msgbuffer, fmt, i, j);
	}
}

Visible Procedure
debug5(string fmt, int i, int j, int k, int l, int m) {
	if (fmt && priority <= 1) {
		priority = 1;
		sprintf(msgbuffer, fmt, i, j, k, l, m);
	}
}

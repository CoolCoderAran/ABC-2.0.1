...
Visible bool help() {
	...
	if (nitems == 0)
		start_help();
	if ((unsigned)llength < (sizeof buffer)-1)
		len = llength+1;
...
Hidden Procedure start_help()
{
	...
	for (h = 0; h < ((sizeof(helpcode))/(sizeof(int))); h++) {
		...
		w= strlen(buffer);
		if ((maxwidth >= 0) && ((unsigned)maxwidth < strlen(buffer)))
			maxwidth= w;
...
Hidden Procedure more_help() {
	...
	int nprinted= 0;
	bool more= Yes;
	int len = ((unsigned)llength < sizeof buffer ? (unsigned)llength+1 : sizeof buffer);

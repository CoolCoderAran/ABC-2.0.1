...
Forward Hidden char *filesuffix(literal type);
...
Visible bool f_interactive(FILE *file) {
	/* TODO: fileno is really non-portable. _POSIX_SOURCE is a hack */
...
Visible value new_fname(value name, literal type) {
	...
	intlet len= sp ? sp - sname : (intlet)strlen(sname);
		/* if a command name only the first keyword */
		/* we can cast strlen(sname) to an intlet since it should never be larger than an intlet in this case*/

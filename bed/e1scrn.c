...
Forward Hidden bool atlinestart(environment *ep);
Forward Hidden int fixlevels(environment *oldep, environment *newep, int highest);
...
/**
 *
 * @param lasttime Yes if called from final screen update
 */
Visible Procedure
actupdate(
	value copybuffer,
	...
	for (p = ptop, curlno = winstart; p && curlno < winheight;
		curlno += Newspace(p), p = p->c_link) {
		++scrlines;
		if (lasttime) {
			p->c_newfocus = No;
			p->c_newvhole = 0;
		}
		if (p->c_onscreen != Nowhere && Newspace(p) == Oldspace(p)) {
			/* Old comrade */
			...
			if (p->c_oldfocus || p->c_newfocus
				...
				|| p->c_onscreen + Newspace(p) >= winheight) {
...
Hidden int makeroom(cell *p, int curlno, int delcnt) {
	register int here = 0;
	register int needed = Newspace(p);
	register int amiss;
	int avail;
	int delta;
...
#ifndef NDEBUG
	if (dflag)
		debug5("[makeroom: curlno=%d, delcnt=%d, here=%d, avail=%d, amiss=%d]",
...
Hidden int make2room(cell *p, int curlno, int delcnt) {
	int nextline = curlno + Newspace(p);
...
Visible Procedure
virtupdate(environment *oldep, environment *newep, int highest) {
	environment old;
	environment new;
	register int oldlno;
	register int newlno;
...
Hidden bool atlinestart(environment *ep) {
...
Hidden int fixlevels(environment *oldep, environment *newep, int highest) {
	...
	if (Ycoord(newep->focus) != Ycoord(oldep->focus) ||
		PLevel(newep->focus) != PLevel(oldep->focus)) {
		/* Inconsistency found.  */
...
backtranslate(int *py, int *px) {
	...
	for (i = 0, p = tops; p; ++i, p = p->c_link) {
		if (p->c_onscreen != Nowhere
			&& y >= p->c_onscreen && y < p->c_onscreen + Newspace(p)) {

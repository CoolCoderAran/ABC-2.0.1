...
Forward Hidden bool isunititem(environment *ep);
Forward Hidden Procedure lastnnitem(environment *ep);
Forward Hidden bool nextitem(environment *ep);
Forward Hidden bool previtem(environment *ep);
...
Visible Procedure
grow(environment *ep, bool deleting) {
	subgrow(ep, Yes, deleting);
}

Visible Procedure
subgrow(environment *ep, bool ignorespaces, bool deleting) {
	register node n;
	register int sym;
	register int i;
	register int len;
...
#ifdef NOT_USED
Visible Procedure
growsubset(environment *ep) {
	subgrsubset(ep, Yes);
}
#endif

Visible Procedure
subgrsubset(environment *ep, bool ignorespaces) {
	register node n = tree(ep->focus);
	register string *rp = noderepr(n);
	register int nch21 = nchildren(n)*2 + 1;
	short i;
...
Visible Procedure
s_downi(environment *ep, int i) {
	if (!downi(&ep->focus, i))
		syserr(MESS(7101, "s_downi failed"));
}

Visible Procedure
s_down(environment *ep) {
	if (!down(&ep->focus))
		syserr(MESS(7102, "s_down failed"));
}

Visible Procedure
s_downrite(environment *ep) {
...
Hidden bool nextitem(environment *ep) {
...
Hidden bool previtem(environment *ep) {
	if (ep->s1 <= 1
...
Hidden bool isunititem(environment *ep) {
	if (ep->s1&1)
		return Yes;
...
Hidden Procedure lastnnitem(environment *ep) {
...
Visible int
focindent(environment *ep) {
	int y = Ycoord(ep->focus);
	int x = Xcoord(ep->focus);
	int level = PLevel(ep->focus);
	node n = tree(ep->focus);
...
/*
 * Routines to move 'environment' structures.
 */

Visible Procedure
emove(environment *s, environment *d) {
#ifdef STRUCTASS
	*d = *s;
#else /* !STRUCTASS */
	d->focus = s->focus;
...
Visible Procedure
ecopy(environment *s, environment *d) {
	emove(s, d);

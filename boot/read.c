...
#define CCOMMENT '#' /* Not ABC-like but very UNIX-like, and we used cpp ... */
#define QUOTE '"'
...
Forward Hidden Procedure add_special_definitions();
Forward Hidden Procedure add_symbol(string name, int *pn, bool referenced);
Forward Hidden Procedure adv();
Forward Hidden string bodyname(string s);
Forward Hidden Procedure classname(string s);
Forward Hidden Procedure getdefinition();
Forward Hidden string getname();
Forward Hidden string getstring();
Forward Hidden Procedure skipdef();
Forward Hidden Procedure skipspace();
Forward Hidden Procedure skipstring();
Forward Hidden Procedure skipwhite();
Forward Hidden Procedure symnames(string s);


Hidden string dname= NULL; /* name currently being defined (at linestart) */
/* VARARGS 1 */
Hidden Procedure error(char *format, char *arg1, char *arg2, char *arg3, char *arg4, char *arg5) {
	fprintf(stderr,
...
/* to synchronize after error in def */
/* assumes at least points are allright */
Hidden Procedure skipdef() {
	while (nextc != '.') {
		adv();
	}
}

/* idem for string, must end with '"' */
Hidden Procedure skipstring() {
	while (nextc != '\"') {
		adv();
	}
}
...
Hidden string getname() {
	...
	if (!chr_isascii(nextc) || !isalpha(nextc)) {
		if (!chr_isascii(nextc) || (!isprint(nextc) && nextc != ' '))
...
Hidden Procedure symnames(string s) {
	string t= s+1;
	char c;

...
Forward Hidden bool rm_dir(char *path);
Forward Hidden Procedure save_curlast(value wskey, value ws);
Forward Hidden bool setbwsdir();
Forward Hidden Procedure setcurdir(char *path);
Forward Hidden bool wschange(value ws);
Forward Hidden Procedure wscurrent();
Forward Hidden Procedure wsempty(value ws);
Forward Hidden Procedure wserr(int m, bool use_cur);
Forward Hidden Procedure wserrV(int m, value v, bool use_cur);
Forward Hidden bool wsinit();
Forward Hidden bool wsp_writable();
...
Visible Procedure initbws() {
	...
	if (is_path(OPTworkspace)) {
		/* !OPTbwsdir already assured in platform.c */

		if (OPTcentral && !is_path(OPTcentral)) {
		        /* error; message already given in platform.c; */
			;
		}

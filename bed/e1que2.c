...
Forward Hidden bool ack_or_kill_varsuggrest(environment *ep, string *pstr);
Forward Hidden bool add_string(environment *ep, string *pstr);
Forward Hidden bool canfitchar(int c, struct classinfo *ci);
Forward Hidden bool fits_kwchar(node n, int i, int ch);
Forward Hidden bool fits_nextkwstart(node n, int ch);
Forward Hidden bool ins_node(environment *ep, node n, queue *pq);
Forward Hidden bool is_varsuggrest(node n, bool exphole_seen);
Forward Hidden bool joinnodes(path *pp, node n1, node n2, bool spflag);
Forward Hidden bool range_hack(environment *ep);
Forward Hidden bool softening_builtin(char inch, int sym);
...
Visible bool
ins_queue(environment *ep, queue *pq, queue *pq2) {
...
Visible bool
app_queue(environment *ep, queue *pq) {
	int where;
...
Hidden bool ins_node(environment *ep, node n, queue *pq) {
	register int sym;
	register node nn;
	register markbits x;
	string *rp;
 ...
 */
Hidden bool ack_or_kill_varsuggrest(environment *ep, string *pstr) {
...
Hidden bool range_hack(environment *ep) {
	path pa;
	int sympa;
	string str;
	node n1;
	...
	if (sympa == List_or_table_display || sympa == List_filler_series) {
		...
		if ((unsigned)s2 == strlen(str) && str[s2-1] == '.') {
			str[s2-1]= '\0';
			n1= gram(Name);
...
Visible bool
ins_string(environment *ep, /*auto*/ string str, queue *pq, int alt_c) {
	register node nn;
	value v;
	char buf[1024];
	...
	while (*str) {
		...
		case VHOLE:
			...
			len = 0;
			if (!ep->spflag) {
				while ((unsigned)len < sizeof buf - 1 && str[len]
...
Hidden bool add_string(environment *ep, string *pstr) {
	register struct table *tp;

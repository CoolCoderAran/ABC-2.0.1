...
int main(int argc, char **argv) {
	(void)argc; // unused parameter warning
	(void)argv;
...
/****** Hidden bit + sanity check ***************************************/
	if (ibase != 10) {
		...
		int numbits = mantbits+iexp+1;
		if ((numbits >= 0) && ((unsigned)numbits == sizeof(double)*bits+1)) { //explicitly check that a is positive before comparing to unsigned number
			printf("%sDouble arithmetic uses a hidden bit%s\n",
			       co, oc);
		} else if ((numbits >= 0) && ((unsigned)numbits == sizeof(double)*bits)) {
...
/****** The point of it all: ********************************************/
	...
	LONGBITS= min(longpower, DBLBITS);

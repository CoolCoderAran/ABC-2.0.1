...
Forward Hidden bool atdedent(environment *ep);
Forward Hidden bool atrealhole(environment *ep);
Forward Hidden bool fiddle(environment *ep, bool *pmayindent, bool reading_file);
Forward Hidden bool hackhack(environment *ep);
Forward Hidden bool nexthole(environment *ep);
Forward Hidden bool quot_in_tag(int c, environment *ep);
...
Hidden bool quot_in_tag(int c, environment *ep) {
...
Visible bool ins_char(environment *ep, int c, int alt_c) {
	queue q = Qnil;
	queue qf = Qnil;
	string str;
	char buf[2];
...
Visible bool ins_newline(environment *ep, bool reading_file) {
	register node n;
	register int sym;
	bool mayindent;
...
Hidden bool fiddle(environment *ep, bool *pmayindent, bool reading_file) {
	register int level;
	string str = "";
	...
	*pmayindent = Yes;
	if (atdedent(ep)) {
		*pmayindent = No;
		s_up(ep);
		level = PLevel(ep->focus);
		delfocus(&ep->focus);
		if (symbol(tree(ep->focus)) == Hole) {
			if (hackhack(ep))
				return Yes;
		}
		while (PLevel(ep->focus) >= level) {
			if (!nexthole(ep)) {
				ep->mode = ATEND;
				break;
			}
...
Hidden bool hackhack(environment *ep) {
	node n;
...
Hidden bool atdedent(environment *ep) {
	register path pa;
	register node npa;
	register int i;
...
Hidden bool nexthole(environment *ep) {
	register node n;
	register int ich;
...
Hidden bool atrealhole(environment *ep) {
	node n;
	int i;
	...
	if (ep->mode == FHOLE
		&& ep->s2 >= 0
	    && strlen(noderepr(n)[i= ep->s1/2]) <= (unsigned)ep->s2) {
		if (i < nchildren(n)) {
			n= child(n, i+1);
			if (Is_etext(n))
				return No;

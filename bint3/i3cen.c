...
/* SP 20260112 `use_env` is already a termcap function */
Visible wsenvptr abc_use_env = &useenv; /* pointer to env of using workspace */
...
Visible Procedure initcentralworkspace(bool startup) {
	...
	if (incentralws) {
		...
		abc_use_env->errlino = cen_env->errlino;
	}
...
Visible Procedure endcentralworkspace(bool last) {
	...
	if (!last) {
		if (incentralws) {
			...
			release(cen_env->perm);
			cen_env->perm = copy(abc_use_env->perm);
			release(cen_env->abctypes);
			cen_env->abctypes = copy(abc_use_env->abctypes);
			cen_env->errlino = abc_use_env->errlino;
		}
		release(cen_env->units);
		cen_env->units = Vnil;
	}
...
/**
 *
 * @param howto if non-NULL set to root of loaded unit
 * @param wse if non-NULL set to workspace env of unit
 */

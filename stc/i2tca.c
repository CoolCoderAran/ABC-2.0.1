...
Forward Hidden Procedure def_typecode(value pname, value tc, wsenvptr wse);
Forward Hidden Procedure del_typecode(value pname, wsenvptr wse);
...
Forward Hidden bool tc_exists(value pname, value **cc, wsenvptr wse);
...
%	pop(u); interchange like ~; int push(u) {       push(mk_elt());
...
Visible Procedure type_check(parsetree v) {
	...
	ext_level= 0;
	nformals= 0;
	if (Unit(n) || Command(n) || Expr(n)) {
		tc_node(v);
		if (!interrupted && Expr(n))
			p_release(pt_pop());
	}

...
Visible extern wsenvptr abc_use_env; /* from bint3/i3cen.c */

Forward Hidden Procedure call(intlet type, parsetree new_pc);
Forward Hidden bool collect_value(value *pv, value seq, value locenv, int *pnextlocal);
Forward Hidden bool extract(loc l, value *pv);
Forward Hidden bool is_location(value v);
Forward Hidden value n_content(loc l);
Forward Hidden Procedure n_put(value v, loc l);
Forward Hidden value n_trim(value v, value B, value C);
Forward Hidden env newenv(envtab tab, env inv_env);
Forward Hidden Procedure popenv(env *pe);
Forward Hidden Procedure pushenv(env *pe);
Forward Hidden Procedure put_it_back(value v, loc l);
Forward Hidden Procedure putbackargs(value locenv);
Forward Hidden Procedure st_grow(int incr);
Forward Hidden Procedure sub_putback(parsetree argtree, value lv, value locenv, int *pnextlo);
Forward Hidden Procedure udfpr(value nd1, value name, value nd2, bool isfunc);
Forward Hidden Procedure sub_epibreer(parsetree argtree, value vl, value *plocals, int *pnextlocal);
Forward Hidden value epibreer(parsetree formals, int argcnt, int nlocals);
...
Hidden dorecord doo[] = {
	...
	/* HOW-TO */	{New, Old, Non, New, HT_Void},
	/* YIELD */	{New, Cpy, Non, Non, HT_Return},
	/* TEST */	{New, Cpy, Non, Non, HT_Report},

	/* REF-CMD */	{Old, Old, Old, Old, HT_Void},
	/* ref-expr */	{Cpy, Cpy, Non, Old, HT_Return},
	/* ref-test */	{Cpy, Cpy, New, Old, HT_Report}
};
...
Hidden Procedure call(intlet type, parsetree new_pc) {
	if (type < 0 || (unsigned)type >= MAXTYPE) syserr(MESS(4101, "bad call type"));
...
Visible Procedure ret() {
	...
	type= Checksum(Ipop());
	if (type < 0 || (unsigned)type >= MAXTYPE) syserr(MESS(4102, "stack clobbered"));
	...
	if (Ipop() == H_using) resetcurenv(abc_use_env);
	...
	if (re == HT_Return && still_ok) push(rv);
}
...
Visible Procedure call_refinement(value name, parsetree def, bool test) {
	...
	call(test ? C_reftest : C_refexp,
		*Branch(FRefinement(def)->rp, REF_START));
}
...
/**
 * @param formals Parse tree for formals
 * @param argcnt Nr. of argument slots
 * @param nlocals Nr. of local variables
 */
...
/**
 * @param plocals Value or location
 */
...
Visible Procedure x_user_command(value name, parsetree actuals, value def) {
	...
	if (Valid(def)) {
		...
		call(C_refcmd, *Branch(FRefinement(def)->rp, REF_START));
		return;
	}

...
Forward Hidden bool delvarying(environment *ep) ;
Forward Hidden bool delfixed(environment *ep) ;
Forward Hidden bool delsubset(environment *ep, bool hack) ;
Forward Hidden int delsublist(environment *ep) ;
Forward Hidden bool delwhole(environment *ep) ;
Forward Hidden bool delhole(environment *ep) ;
Forward Hidden Procedure nonewline(queue *pq) ;
Forward Hidden Procedure balance(environment *ep) ;
Forward Hidden bool copyin(environment *ep, queue q) ;
Forward Hidden bool colonhack(environment *ep, bool all) ;
...
Hidden bool delvarying(environment *ep) {
	queue q = Qnil;
...
Hidden bool delfixed(environment *ep) {
	register node n = tree(ep->focus);
	...
	Assert(fwidth(repr) < (int)(sizeof buf - 1));
		/*buf has size 15, always fits in int*/
	len = ep->s2;
	ep->s2 = ep->s3 + 1;
	ep->mode = FHOLE;
	nosuggtoqueue(ep, &q);
...
Hidden bool hole_ify_keywords(environment *ep, queue *qq) {
...
Hidden bool delsubset(environment *ep, bool hack) {
	queue q = Qnil;
	queue q2 = Qnil;
	register node n = tree(ep->focus);
	register node nn;
...
Hidden int delsublist(environment *ep) {
	register node n;
	register int i;
	register int sym;
	queue q = Qnil;
...
Hidden bool delwhole(environment *ep) {
...
Hidden bool delhole(environment *ep) {
	node n;
	int sym;
	bool flag = No;
...
Visible bool copyinout(environment *ep) {
	shrink(ep);
	if (!ishole(ep)) {
		release(ep->copybuffer);
		ep->copybuffer = copyout(ep);
...
Visible value copyout(environment *ep) {
	queue q = Qnil;
	path p;
	register node n;
	register value v;
	...
	case SUBRANGE:
		...
		if (ep->s1&1) { /* Fixed text */
			Assert(ep->s3 - ep->s2 + 1 < (int)(sizeof buf));
...
Hidden Procedure balance(environment *ep) {
...
Hidden bool copyin(environment *ep, /*auto*/ queue q) {
	queue q2 = Qnil;
	bool res;
...
Hidden bool colonhack(environment *ep, bool all) {

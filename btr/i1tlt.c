...
Hidden value treesize(btreeptr pnode) {
	...
	case CBottom:
	case Crange:
	    syserr(MESS(327, "Bigsize in CBottom or Crange"));
		break;
	}
	return(vsize);
    }
...
Visible value item(value v, value num) { /* v item num */
	value m= Vnil;
	if (!Is_tlt(v))
		interr(ITEM_TLT);
...
Visible value th_of(value num, value v) { /* num th'of v */
	value m= Vnil;
	if (!Is_tlt(v))
		interr(THOF_TLT);
	else if (!Is_number(num))
...
Hidden intlet it;		/* item type of t*/
...
Hidden bool walktree(btreeptr p, bool (*visit)(union itm *)) {
	intlet l;
	...
	for (l=0; l < Lim(p); l++) {
		...
		case Inner:
			if (!walktree(Ptr(p, l), visit) || !still_ok)
				return No;
			if (!(*visit)(Piitm(p, l, it)) || !still_ok)
				return No;
			break;
		case CBottom:
			if (!(*visit)(Pbitm(p, l, it)) || !still_ok)
				return No;
		}
	}
	return Flag(p) EQ CBottom || walktree(Ptr(p, l), visit);
}
 ...
 * @param e [e] func t
 * @param t
 * @param li_func func for lists
 * @param te_visit 'visit' for walktree
 * @param ta_visit 'visit' for walktree
 */
Hidden int tlt_func(
	value e, value t,
	value (*li_func)(struct value *, struct value *),
	bool (*te_visit)(union itm *),
	bool (*ta_visit)(union itm *)
) {
	m_val = Vnil;
	if (empty(t)) {
		tlterr= T_EMPTY;
...
/* FIXME: This function seems to segfault on the MUSL statically-linked binary.

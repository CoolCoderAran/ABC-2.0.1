...
Hidden Procedure readtermcap() {
	char buffer[2048]; /* Constant dictated by termcap manual entry */
...
#ifdef TERMIO
	struct termio sgbuf;
#define GTTY(f,s) ioctl((f), TCGETA, (char*)(s))
#endif
#ifdef TERMIOS
	struct termios sgbuf;
#define GTTY(f,s) tcgetattr((f), (s))
#endif
#ifndef SGTTY /* TERMIOS or TERMIO */
	static char str[6];
...
#ifdef VSUSP
    /* FIXME: VSUPS is actually out of range for array */

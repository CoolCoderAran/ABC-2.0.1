...
Forward Hidden Procedure convbtext(Procedure (*outproc)(char), btreeptr p, char quote);
...
Hidden btreeptr mkbtext(string s, int len) {
	...
	p = grabbtreenode(chunk ? Inner : CBottom, Ct);
	Size(p) = len;
	Lim(p) = n;
	if (!chunk)
...
typedef finger stack;
typedef fingertip stackptr;
...
#define q2 newptr[1]
	...
	while (s1 < sp1 || s2 < sp2) {
		...
		if (n > (inner ? Maxinner : Maxbottom)) {
			...
			underflow = Yes;
		q1 = grabbtreenode(inner ? Inner : CBottom, Ct);
		Lim(q1) = n;
		cp = cbuf;
		...
		if (overflow) {
			newitem = *cp++;
			q2 = grabbtreenode(inner ? Inner : CBottom, Ct);
			Lim(q2) = n2;
...
Visible Procedure convtext(Procedure (*outproc)(char), value v, char quote) {
...
Hidden Procedure convbtext(Procedure (*outproc)(char), btreeptr p, char quote) {

...
Forward Hidden Procedure backpatch(parsetree t);
Forward Hidden Procedure endthreads(parsetree *code);
Forward Hidden Procedure f_actuals(parsetree formals, parsetree actuals);
...
Forward Hidden bool is_basic_target(value v);
Forward Hidden bool is_cmd_ref(parsetree t);
Forward Hidden bool is_target(parsetree *p);
Forward Hidden Procedure jump2here(parsetree t);
Forward Hidden Procedure jumphere(parsetree t);
Forward Hidden Procedure lvisit(parsetree t);
Forward Hidden Procedure no_mysteries(void);
Forward Hidden bool reachable(void);
Forward Hidden parsetree seterr(int n);
Forward Hidden Procedure sk_tsuite_comment(parsetree v, parsetree *w);
Forward Hidden bool trim_opr(value name, typenode *type);
Forward Hidden Procedure visit(parsetree t);
Forward Hidden Procedure visit2(parsetree t, parsetree t2);

Visible Procedure fix_nodes(parsetree *pt, parsetree *code) {
	context c; value *setup(parsetree pt), *su; //TODO put setup in header file so it can be neatly included
	sv_context(&c);

...
Forward Hidden node adv_howsugg(environment *ep, char prev_c, char new_c);
Forward Hidden bool checksugg(string bp);
Forward Hidden Procedure delsugg(string str);
Forward Hidden node firstsugg(string s, int startsugg, bool colon_allowed);
Forward Hidden bool fits_how_to(string str, string *pstr, int alt_c);
Forward Hidden bool getpattern(node n);
Forward Hidden Procedure getsugg(char *sgfile);
Forward Hidden node nextsugg(string str, int len, int new_c, bool in_sugghowname, bool colon_allowed);
Forward Hidden bool res_firstkeyword(string str);
...
Visible Procedure addsugg(string str, int builtin) {
	...
	for (i = nsugg-1; i >= 0; --i) {
		...
		if (suggi[len]
		    ...
		    (suggi[len] != ' '
		     ||
		     (chr_isascii(suggi[len+1]) && isupper(suggi[len+1]))
		    )
		)
...
Visible bool setsugg(path *pp, char c, environment *ep, bool colon_allowed) {
	char buf[2];
	node n;
	string s;
	string lastunitname();
...
Hidden node adv_howsugg(environment *ep, char prev_c, char new_c) {
	int s2= ep->s2;
	char buf[2];
	value hd;
	value tl;
...
Visible Procedure killhowsugg(environment *ep) {
	...
	hd= e_icurtail((value) firstchild(tree(ep->focus)), s2);
	s_down(ep);
	treereplace(&ep->focus, nodecopy((node)hd));
	s_up(ep);
}

...
#ifdef TERMIOS
Hidden struct termios oldtty, newtty, polltty;
#define gtty(fd,bp) tcgetattr(fd, (struct termios *) bp)
#define stty(fd,bp) VOID tcsetattr(fd, TCSADRAIN, (struct termios *) bp)
#endif /* TERMIOS */
...
Hidden FILE *fp= NULL;
Forward Hidden int outchar(int ch);
...
Forward Hidden int strcost(char *str);
Forward Hidden int subshell();
Forward Hidden SIGTYPE trmintrhandler(int sig);
...
// NOTE: gtty is not actually implemented in the Linux kernel. See gtty(2)
Hidden int setttymode() {
	if (!know_ttys) {
...
#endif
#else /* TERMIO/S */
		/* NOTE: cfgetospeed cannot be polyfilled by polyfill-glibc. */
		// ospeed = cfgetospeed(&oldtty);
		...
		newtty.c_cc[VTIME]= 0; /* or 0.1 sec. */
	...
	newtchars.t_intrc= intrchar;   /* do not disable interrupt */

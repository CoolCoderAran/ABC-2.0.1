...
Forward Hidden bool canexit(environment *ep);
Forward Hidden bool execute(environment *ep, int cmd);
...
Visible bool
editdocument(environment *ep, bool bad_file) {
	int k;
	int first = 0;
	int last = 0;
	int current = 0;
	...
	for (;;) { /* Command interpretation loop */
		...
		if (!canceled && trmavail() <= 0) {
			if (onscreen != current)
				virtupdate(onscreen < 0 ? (environment*)NULL : &history[onscreen],
					&history[current],
					reverse && onscreen >= 0 ?
	...
	dumpev(&newenv, "AFTER EXECUTE");
		...
		if (errors
			...
			if (!OPTslowterminal && chr_isascii(cmd)
				&& (isprint(cmd) || cmd == ' '))
				ederrC(INS_BAD, cmd);
			else
				ederr(0);
...
Hidden bool
execute(environment *ep, int cmd) {
...
#define KILLSUGG(ep) /* NULL */
	...
	if (justgoon)
		justgoon = chr_isascii(cmd) && islower(cmd);
	...
	default:
		if (!chr_isascii(cmd) || !isprint(cmd))
			return No;
		ep->spflag = spflag;
...
dbmess(environment *ep) {
	...
	sprintf (messbuf,
		...
		Ycoord(ep->focus), Xcoord(ep->focus), PLevel(ep->focus),
			ep->spflag ? "spflag on" : "");
#else /* !SAVEBUF */
	sprintf (messbuf,
...
Visible Procedure dumpev(environment *ep, string m) {
	char stuff[80];
	register string str = stuff;
	path pa;
	node n;

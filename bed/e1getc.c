...
Forward Hidden string defstring(string name);
Forward Hidden bool equalhead(string keystr, int nkey, string def, int len);
Forward Hidden Procedure err(int m);
Forward Hidden Procedure err1(string m);
Forward Hidden Procedure get_definition();
Forward Hidden Procedure get_line();
Forward Hidden string getname();
Forward Hidden string getrep();
Forward Hidden int getstring(string *pstr);
Forward Hidden Procedure initmouse();
Forward Hidden Procedure initsense();
Forward Hidden bool isIntrBinding(int code, int deflen, string def);
Forward Hidden int lookup(string name);
Forward Hidden Procedure outstring(string name);
Forward Hidden Procedure readkeysfile();
Forward Hidden Procedure setdeflen();
Forward Hidden Procedure sigundef_intrchar();
...
#define COMMENTCHAR '#' /* Not B-like but very UNIX-like */
#define QUOTE '"'
...
/**
 * return whether stored
 */
Hidden bool store(int code, int deflen, string def, string rep, string name) {
	struct tabent *d, *last= deftab+ndefs;
#ifndef CANLOOKAHEAD
	int i;
#endif
...
#define ERR3 MESS(6503, "Definition for command %s starts with '%c'.")
		if (chr_isascii(*def) && (isprint(*def) || *def==' ')) {
...
Hidden string getname()
{
	...
	*bp++= nextc;
	...
	while ((chr_isascii(nextc) && isalnum(nextc))
	       || nextc == '_' || nextc == '-'
	      ) {

...
Forward Hidden Procedure repwidth(node *pn, Register node old, Register node new);
...
Visible node
newnode(int nch, Register int sym, node children[]) {
...
#define Locchild(pn, i) \
	((void)(Refcnt(*(pn)) == 1 || (nodeuniql(pn), 1)), &Child(*(pn), i))
#define Setmarks(pn, x) \
	((void)(Refcnt(*(pn)) == 1 || (nodeuniql(pn), 1)), Marks(*(pn))=(x))
#define Setwidth(pn, w) \
    ((void)(Refcnt(*(pn)) == 1 || (nodeuniql(pn), 1)), Width(*(pn))=w)
...
Visible Procedure
setchild(node *pn, int i, Register node n) {
	register node *pch;
...
Visible path
newpath(path pa, node n, Register int i) {
	register path p = (path) mk_path();

	Parent(p) = pa;
	Tree(p) = n;
	Ichild(p) = i;
	Ycoord(p) = Xcoord(p) = PLevel(p) = 0;
	return p;
}
...
#define Uniqp(pp) ((void)(Refcnt(*(pp)) == 1 || (pathuniql(pp), 1)))
...
Hidden Procedure
repwidth(node *pn, Register node old, Register node new) {
...
Visible Procedure
putintrim(value *pn, int head, Register int tail, Register string str) {
	register value v = *pn;
	value t1, t2, t3;
	int len= e_length(v);
